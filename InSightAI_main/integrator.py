...
def get_slm_records(slm_data):
    ...
    raise ValueError(
        "SLM output must contain a 'results' or 'extractions' list."
    )
...
def build_joined_records(raw_records, slm_records):
    ...
    for slm in slm_records:
        ...
        joined.append({
            ...
            "relevant_time": extraction.get("relevant_time"),
            "direction": extraction.get("direction", "unknown"),
...
def aggregate_evidence(joined_records):
    ...
    for (signal_theme, region, product_category), records in groups.items():
        ...
        severity_counts = Counter(record.get("severity") for record in records if record.get("severity"))

        # ------------------------------------------------------------
        # Dates
        # ------------------------------------------------------------

        dates = sorted(parse_date(record["date"]) for record in records)

        date_start = dates[0].isoformat()
        date_end = dates[-1].isoformat()

        daily_counts = Counter(parse_date(record["date"]).isoformat() for record in records)
        ...
        evidence.append({
            ...
            "observation_trend": calculate_trend(daily_counts),
...
def main():
    ...
    raw_records = load_jsonl(RAW_DATA_PATH)
    print(f"Raw records: {len(raw_records)}")

    print(f"\nReading SLM output:\n  {SLM_OUTPUT_PATH}")

    slm_data = load_json(SLM_OUTPUT_PATH)

    slm_records = get_slm_records(slm_data)

    print(f"SLM records: {len(slm_records)}")

    print("\nJoining raw records with SLM extraction...")

    joined_records = build_joined_records(raw_records,slm_records)

    print(f"Joined records: {len(joined_records)}")
    ...
    evidence = aggregate_evidence(joined_records)
