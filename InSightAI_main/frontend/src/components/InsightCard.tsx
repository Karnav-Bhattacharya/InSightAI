...
export const InsightCard: React.FC<InsightCardProps> = ({
  ...
  const getRecommendationData = (
    ...
    const explicitFeasibility =
      ...
      ||
      (
        insight.actionFeasibilities &&
        insight.actionFeasibilities[index]
      )
      ||
      (
        index === 0
    ...
    return {
      text,
      feasibility: explicitFeasibility
    };
  ...
  const getFeasibilityBadgeStyle = (
    feasibility: FeasibilityLevel
  ) => {
  ...
  const handleCopy = () => {
    ...
    const text =
      `InSightAI Recommendation: ${insight.title} (${insight.region})\n` +
